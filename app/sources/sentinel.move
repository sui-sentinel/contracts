...
const WITHDRAWAL_LOCK_PERIOD_MS: u64 = 1209600000;             // 14 days
...
public struct RegisterAgentResponse has copy, drop {
    ...
    system_prompt: String,
    is_defeated: bool,
    creator: address // Kept in signature for traceability, but not used for routing funds
}
...
public fun request_attack(
    ...
    let amount_to_agent = total_amount - amount_to_owner - amount_to_protocol;

    let mut payment_balance = coin::into_balance(payment);

    // 1. Owner Fee: Stored in the object, claimable via AgentCap
    if (amount_to_owner > 0) {
        let owner_balance = balance::split(&mut payment_balance, amount_to_owner);
        balance::join(&mut agent.accumulated_fees, owner_balance);
    };
    
    // 2. Protocol Fee: Sent immediately to protocol wallet
...
fun test_register_agent_flow() {
    ...
    use enclave::enclave::{new_cap, EnclaveConfig};
    ...
    let cap = new_cap(SENTINEL {}, ctx(&mut scenario));
